...
export class DepartureGp {
  ...
  constructor(props: CreateDepartureProps | DepartureWithRelations) {
    ...
    this.personId             = props.personId ?? undefined
    this.creatorId            = props.creatorId
    this.insurancePrice       = props.insurancePrice ?? undefined
    if ('isClosed' in props)   this.isClosed    = props.isClosed
    else                       this.isClosed    = false
    if ('arrivalDate' in props) this.arrivalDate = props.arrivalDate ?? undefined
    ...
    if ('person' in props)     this.person      = props.person ?? undefined
    if ('creator' in props)    this.creator     = props.creator
    if ('packages' in props)   this.packages    = props.packages
  }
